...
A CellDataChild keeps its parent's <Graphics> -- and therefore its <Edge> --
only when it declares no <Graphics> element at all. Declaring <Graphics> gives
the child its own CellGraphicsInfo, which carries no edges unless it declares
an <Edge> of its own: 63 declared blocks resolve to 66 materials, and the 34
children that declare <Graphics> without <Edge> (wood_tree, rock_static_noedge,
sand_surface, ...) get none. Live-verified against BAKEDUMP baked colors.
...
def parse_materials(xml_path):
    ...
    def edge_of(m, depth=0):
        ...
        g = m.find('Graphics')
        if g is not None:
            # Declaring <Graphics> gives the material its own CellGraphicsInfo:
            # it carries exactly the <Edge> declared there, and NOTHING when it
            # declares none. Only a material with no <Graphics> element at all
            # keeps its parent's graphics, edges included. Verified live against
            # baked cell colors (BAKEDUMP, seed 786433191): the giant tree's
            # wood_tree -- own <Graphics>, no <Edge>, parent rock_static -- has
            # 0 of 25757 cells carrying rock_static's edge palette while
            # rock_static is dressed in the same rect, and the desert's
            # sand_surface / sandstone_surface / sand_static_bright carry none
            # of sand's / sand_static's. The names rock_static_noedge and
            # templebrick_thick_static_noedge -- children of edged parents that
            # declare <Graphics> without <Edge> -- say the same thing.
            return g.find('Edge')
        p = by_name.get(m.get('_parent'))
...
// Noita's per-material <EdgeGraphics> decal tables plus the rects of the
// edge_files sprites packed into data/edge_atlas.bin. A material appears here
// only if it declares an <Edge> itself, or inherits one by declaring no
// <Graphics> element at all -- see the generator's docstring.
//
