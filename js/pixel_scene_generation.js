...
export function loadPixelScene(biomeData, biomeName, sceneName, ws, ng, x, y, skipCosmeticScenes = true, checkBounds = true, gameMode = 'normal') {
	// The biome this scene is being stamped FOR, kept before the recolor logic
	// below rewrites biomeName to "general". Several biomes stamp one scene PNG
	// with different background art (SCENE_BACKGROUNDS_BY_BIOME), and the scene
	// key alone cannot tell them apart -- nine biomes share "general/essenceroom".
	const spawnBiome = biomeName;
	...
	*/
	...
	return {
		...
		name: sceneName,
		// This biome's own background art when it differs from the scene's
		// default; null means "whatever PIXEL_SCENE_DATA carries for the key".
		backgroundArt: SCENE_BACKGROUNDS_BY_BIOME[spawnBiome]?.[sceneName] ?? null,
	...
	for (const scene of scene_list) {
		...
		if (r <= scene.prob) {
			...
			let outputScene = {
				...
				// This biome's own background art when it differs from the scene's
				// default; null means "whatever PIXEL_SCENE_DATA carries for the key".
				backgroundArt: SCENE_BACKGROUNDS_BY_BIOME[biomeName]?.[scene.name] ?? null,
				name: scene.name, 
